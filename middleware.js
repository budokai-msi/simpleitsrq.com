...
async function logThreat(request, geo) {
  try {
    ...
    await sql`
      INSERT INTO threat_actors (
        ...
        ${JSON.stringify(headersObj)}::jsonb, 'hostile_geo'
      )
    `;
  } catch (err) {
    // Best-effort — never block the response on a DB failure.
...
export default function middleware(request) {
  ...
  if (!HOSTILE_COUNTRIES.has(country)) return;
  ...
  // Fire-and-forget threat logging. Don't await — respond immediately.
  request.waitUntil?.(logThreat(request, geo)) ?? logThreat(request, geo).catch(() => {});

  // API calls from hostile geo get a generic error, not the honeypot HTML.
  ...
  // Page navigations get the honeypot.
