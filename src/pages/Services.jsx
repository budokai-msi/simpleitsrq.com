...
function ServiceCard({ svc }) {
  return (
    <article id={svc.slug} className="svc-card reveal-up">
      ...
      <div className="svc-cta"><Link to={svc.slug === "managed-it" ? "/book?topic=managed-it" : "/book?topic=computer-repair"} className="btn btn-primary svc-buy-btn" onClick={()=>trackEvent("generate_lead",{source:"services",service_slug:svc.slug})}>Tell us what you need <ArrowRight size={16}/></Link></div>
    </article>
  );
}
...
export default function Services(){
  ...
  return <main id="main" className="services-main">
    <section className="section services-hero"><div className="container"><span className="eyebrow">Computer repair & business IT</span><h1 className="display">Repair when something fails. Ongoing IT support when the business needs it.</h1><p className="lede">You do not need to know the technical cause before calling. Tell us what is not working, what changed, and what the problem is costing you in time or disruption. We will help narrow down the right kind of work.</p><div className="services-trust-row"><span><MapPin size={14}/> Sarasota / Bradenton area</span><span><ShieldCheck size={14}/> Scope and next steps explained before the work grows</span></div></div></section>
