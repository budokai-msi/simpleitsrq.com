...
title: "Free Exposure Scan for Florida Small Businesses"
metaDescription: "Run a free passive exposure scan for DNS, SPF, DMARC, DKIM, and exposed subdomains. Get a plain-English risk grade in seconds."
date: "2026-04-24"
...
**2. SPF (Sender Policy Framework).** Without SPF, anyone in the world can send email *claiming to be from your domain* and Gmail/Outlook will accept it. We check whether you have an SPF record AND whether it's actually enforcing (`~all`r `-all`instead of the soft `?all`hat does nothing.
...
**5. Subdomain exposure via Certificate Transparency.** Every TLS certificate ever issued for your domain is logged publicly in CT logs. We pull the list and show you what's there. If you have `staging.yourcompany.com` `vpn.yourcompany.com`posed and you forgot, that's a target. Most small businesses don't realize this list exists.
...
- **No MX records**, our `hello@simpleitsrq.com` was bouncing every contact form submission for two weeks. (We didn't notice immediately because forms still SAVED to the database; only the email-out leg was broken.)
- **DMARC at `p=none`nitoring only, not blocking spoofers
- **No IPv6**, cosmetic
...
- Tightening SPF to  or`~all` pr`-all` without breaking legitimate senders
- Walking DMARC from  `p=none``p=quarantine``p=reject`ew weeks of monitoring
...
- [Computer Repair in Sarasota: When to Fix It, When to Replace It, and How to Spot a Scam Quote (2026)](/blog/computer-repair-sarasota-honest-guide-2026)
